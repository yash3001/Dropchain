const Dropchain = artifacts.require("./Dropchain.sol");

require("chai").use(require("chai-as-promised")).should();

contract("Dropchain", ([deployer, uploader]) => {
  let Dropchain;

  before(async () => {
    Dropchain = await Dropchain.deployed();
  });

  describe("deployment", async () => {
    it("deploys successfully", async () => {
      const address = await Dropchain.address;
    ...
    it("has a name", async () => {
      const name = await Dropchain.name();
      assert.equal(name, "Dropchain");
    });
  });
  ...
  describe("file", async () => {
    ...
    before(async () => {
      result = await Dropchain.uploadFile(
        fileHash,
        fileSize,
        fileType,
        fileName,
      ...
      fileCount = await Dropchain.fileCount();
    });
    ...
    it("upload file", async () => {
      ...
      await Dropchain.uploadFile(fileHash, fileSize, fileType, fileName, "", {
        from: uploader,
      }).should.be.rejected;
    });
    ...
    //check from Struct
    it("lists file", async () => {
      const file = await Dropchain.files(fileCount);
      assert.equal(
